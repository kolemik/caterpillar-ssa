...
public class SSAdebug
{
    private Logger log = LoggerFactory.getLogger(SSAdebug.class);
    ...
    * */
    private double[] calculateForecasting(int T, int L, int N, int M, Matrix U, Matrix Q)
    {
    	...
    	U.print(5, 3);
    	System.out.println();
    	System.out.println("Q: " + Q.getRowDimension() + " / " + Q.getColumnDimension());
    	Q.print(5, 3);
    	System.out.println();
        ...
        double v = UM.norm2();
        A.timesEquals(1.0 / (1.0 - Math.pow(v, 2.0)));
        A = matrixColumnRevert(A);
        Matrix G = new Matrix(Q.getRowDimension(), 1);
        for (int i = 0; i < G.getRowDimension(); i++)
        {
            double sum = 0;
            for (int j = 0; j < N; j++) // reduce usage of eigen components by N
            {
                sum = sum + Q.get(i, j);
            }
            G.set(i, 0, sum);
        }
        System.out.println("G: " + G.getRowDimension() + "/" + G.getColumnDimension());
        G.print(5, 3);
        System.out.println();
        ...
        System.out.println("A: " + A.getRowDimension() + " / " + A.getColumnDimension());
        ...
        for (int i = T; i < T + M; i++)
        {
            ...
            for (int j = 1; j < L; j++)
            {
                res = (res + (A.get(j - 1, 0) * F.get(i - j, 0)));
            }
            F.set(i, 0, res);
        }
...
//        System.out.println("traject: " + trajectoryMatrix.getRowDimension() + " / " + trajectoryMatrix.getColumnDimension());
        ...
        System.out.println("EIGEN VALUES: " + Arrays.toString(singularValueDecomposition.getSingularValues()));
